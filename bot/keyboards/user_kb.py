...
def get_main_menu_kb() -> InlineKeyboardMarkup:
    kb = InlineKeyboardBuilder()
    kb.button(text='🔍 Поиск матча', callback_data=f'{CB.SEARCH_START.value}')
    kb.button(text='🔥 Топ-прогнозы', callback_data=f'{CB.SEARCH_GET_RESULT.value}:{Action.TOP.value}')
    return kb.adjust(1).as_markup()
...
def get_sports_kb(sports: list[str]) -> InlineKeyboardMarkup:
    ...
    for sport in sports:
        ...
        kb.button(text=title, callback_data=f'{CB.SEARCH_TOURNAMENT.value}:{sport}')
...
def get_tournaments_kb(tournaments: list[str]) -> InlineKeyboardMarkup:
    ...
    for tournament in tournaments:
        kb.button(text=tournament, callback_data=f'{CB.SEARCH_GET_RESULT.value}:{tournament}')
    kb.button(text='🔙 Назад', callback_data=f'{CB.SEARCH_START.value}')
    return kb.adjust(1).as_markup()


# Кнопки подписаться на канал
def get_forecast_kb(back_start: bool) -> InlineKeyboardMarkup:
    kb = InlineKeyboardBuilder()
    kb.button(text='💰 Сделать ставку', url=conf.bet_url)
    if back_start:
        kb.button(text='🔙 Назад', callback_data=f'{CB.COM_START.value}:{Action.BACK.value}')
    else:
        kb.button(text='🔙 Назад', callback_data=f'{CB.SEARCH_TOURNAMENT.value}:{Action.BACK.value}')
    return kb.adjust(1).as_markup()